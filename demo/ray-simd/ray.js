...
//
// The language is Javascript+SIMD.  This runs in Firefox Nightly, at
// least, but only at about the same speed as the non-SIMD version.
// The dot product can probably be improved, but even if its share
// were to fall from 25% to 0% we'd just be neck-and-neck with the
// plain-JS flatjs version.
...
function DL3(x, y, z) { return SIMD.float32x4(x,y,z,0); }

function add(a, b)    { return SIMD.float32x4.add(a, b); }
function addi(a, c)   { return SIMD.float32x4.add(a, SIMD.float32x4(c,c,c,0)) }
function sub(a, b)    { return SIMD.float32x4.sub(a, b); }
function subi(a, c)   { return SIMD.float32x4.sub(a, SIMD.float32x4(c,c,c,0)) }
function muli(a, c)   { return SIMD.float32x4.mul(a, SIMD.float32x4(c,c,c,0)) }
function divi(a, c)   { return SIMD.float32x4.div(a, SIMD.float32x4(c,c,c,0)) } // Note, do we get NaN in the last slot now?
function neg(a)       { return SIMD.float32x4.neg(a); } // Note, do we get -0 in the last slot now?
function length(a)    { var v = SIMD.float32x4.mul(a,a);  return Math.sqrt(v.x + v.y + v.z); }
function normalize(a) { var d = length(a); return SIMD.float32x4.div(a, SIMD.float32x4(d,d,d,0)) }
function cross(a, b)  { return SIMD.float32x4(a.y*b.z - a.z*b.y, a.z*b.x - a.x*b.z, a.x*b.y - a.y*b.x, 0); }
